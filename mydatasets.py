...
class CSVDataset(TarDataset):
    url = 'https://www.cs.cornell.edu/people/pabo/movie-review-data/rt-polaritydata.tar.gz'
    filename = 'sarcasm-10k.csv'
    dirname = 'yelp-sentiment'
    ...
    def __init__(self, text_field, label_field, options=1, header=True, path=None, examples=None, **kwargs):
        ...
        #pdb.set_trace()
        super(CSVDataset, self).__init__(examples, fields, **kwargs)
        #pdb.set_trace()
    ...
    def splits(cls, text_field, label_field, train_filepath, test_filepath, options=1, header=True, dev_ratio=.1, shuffle=True, root='.', **kwargs):
        ...
        test_examples = cls(text_field, label_field, path=test_filepath, options=options, header=header, **kwargs)
